...
The browser dashboard reads the same local Career-Ops tracker, report, pipeline,
batch state, and setup files used by the existing modes:
...
The web dashboard can add URLs to the pipeline inbox, preview linked reports,
and update statuses for existing tracker rows. Its `Resume Workflow` page keeps
the current guided flow in the dashboard and links out to the Phase 2 resume lab
for upload, ATS, rewrite, and evidence work. It does not submit applications or
replace the agent-driven evaluation, scan, PDF, and application-assist modes.

Career-Ops can be driven from an agent CLI path such as Codex, Claude Code, or
Gemini CLI when that CLI is installed and authenticated locally.
